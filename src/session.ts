...
function showSavedPrefToast(data: string): string {
    Toast.show({
        text: "✓ chess-online.com: " + i18n("yourPreferencesHaveBeenSaved"),
        position: "center",
        duration: "short",
    });
    return data;
...
function lichessBackedProp<T extends PrefValue>(
    path: string,
    defaultVal: T
): Prop<T> {
...
function confirmEmail(token: string): Promise<Session> {
    return fetchJSON<Session>(`/signup/confirm/${token}`, undefined, true).then(
        (data) => {
            session = data;
            storeSession(data);
            return session;
...
function rememberLogin(): Promise<Session> {
    return fetchJSON<Session>("/account/info").then((data) => {
        session = data;
        storeSession(data);
        return data;
    });
